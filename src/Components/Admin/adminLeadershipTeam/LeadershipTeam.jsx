...
//  Api
import { getAllCencomUser } from "../../../services/cencom/cencomUser.services";
...
function LeadershipTeam() {
  const [cencomUser, setCencomUser] = useState([]);
  ...
  const getCencomUsers = useCallback(() => {
    (async () => {
      setLoader(true)
      const lgAdmin = "?is_lg_admin=1";
      const paginate = "&page=" + currentPage + "&limit=" + pageSize;

      const response = await getAllCencomUser({ lgAdmin, paginate });
      if (response.status) {
        setTotalRecords(response.data.count);
        setCencomUser(response.data.data)
      }
      setLoader(false)
    })();
    ...
    <>
      ...
      <main className="main flex flex-col flex-grow -ml-64 md:ml-0 transition-all duration-150 ease-in">
        <div className="main-content flex flex-col flex-grow">
          <div className="lg:p-10 bg-about text-center xs:p-[22px] xs:mt-[2px]">
            <h1 className="text-center mont-serif font-semibold text-2xl text-[#005125] ml-12">
              Leadership Team
            </h1>
          </div>
          <div className="lg:w-[80%] mx-auto overflow-auto mt-[2.5rem]">
            <Box height="90vh">
              <DataGrid
                rows={cencomUser.map((row, index) => ({ ...row, index: pageIndex + index + 1 }))}
                columns={columns}
                pagination
                pageSize={pageSize}
          ...
          <div className="button mx-auto flex m-10">
